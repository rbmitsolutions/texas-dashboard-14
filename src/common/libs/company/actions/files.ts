...
export const getFiles = async (params: IGETFilesQuery): Promise<IFiles | IGETCompanyAllFilesResponse> => {
    ...
    const paramsUrl = new URLSearchParams({
        files: JSON.stringify(params)
    })

    const data = await api.get(`${EndPointsTypes['COMPANY_FILES_ENDPOINT']}?${paramsUrl}`, {
        method: 'GET',
        headers: {
